  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (!user) {
      try {
        ...
        // Notify owner of new user created via session sync (fire-and-forget)
        notifyOwner({
          title: "🎉 New User Signed Up — Meeting Agent",
          content: `A new user joined via session sync.\n\n**Name:** ${userInfo.name || "Unknown"}\n**Email:** ${userInfo.email || "Not provided"}\n**Time:** ${new Date().toLocaleString("en-MY", { timeZone: "Asia/Kuala_Lumpur" })} (MYT)`,
        }).catch(err => console.warn("[Notification] New user (session sync) notify failed:", err));
      } catch (error) {
